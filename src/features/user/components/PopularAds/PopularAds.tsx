        ...
        <section className="container mx-auto px-6 py-16">
            <div className="mb-10 text-center">
                <h2 className="text-4xl font-extrabold text-gray-900 tracking-tight mb-4">
                    Most Popular <span className="text-[var(--color-adminMainColor)]">Rooms</span>
                </h2>
                <p className="text-gray-500 text-lg max-w-2xl mx-auto">
                    Discover our most highly rated and frequently booked rooms, carefully curated for an unforgettable stay.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {ads.slice(0, 6).map((ad) => (
                    <div
                        key={ad._id}
                        className="relative overflow-hidden rounded-2xl group shadow-lg h-[320px] bg-white border border-gray-100"
                    >
                        {/* Image */}
                        <img
                            src={ad.room.images?.[0] || "https://via.placeholder.com/600"}
                        ...
                        <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-md text-gray-900 px-4 py-1.5 rounded-full font-bold shadow-lg text-sm z-10 flex items-center gap-1">
                            <span className="text-[var(--color-adminMainColor)]">${ad.room.price}</span> / night
                        </div>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center gap-4 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0 z-20">
                            {isUserRole && (
                                <button
                                    onClick={() => handleAddToFavorites(ad.room._id)}
                            ...
                            <div className="flex justify-between items-end">
                                <div>
                                    ...
                                    <p className="text-gray-300 text-sm font-medium">
                                        Premium Suite • {ad.room.capacity} Guests
                                    </p>
                                </div>
                                {ad.room.discount > 0 && (
